 ...
 * - 메인쿼리(== 기존쿼리 , 외부 쿼리)를 위해 보조 역할을 하는 쿼리문
 * 
 * - 메인쿼리가 SELECT 문일 때
...
-- 전 직원의 급여 평균보다 많은(초과) 급여를 받은 직원의
...
--3) 위의 두 쿼리를 합쳐 부서별 급여 합이 큰 부서의 부서명, 급여 합 조합
SELECT DEPT_TITLE , SUM(SALARY)
FROM EMPLOYEE 
LEFT JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
GROUP BY DEPT_TITLE
...
-- 2) DEPARTMENT 테이블의 위의 결과(L1)과 동일한 LOCATION_ID를 가지고있는 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
--> 상관 서브쿼리를 사용하여 각 직원의 MANAGER_ID가 실제로 
--  직원 테이블의 EMP_ID와 일치하는지 확인
...
SELECT  EMP_ID
FROM EMPLOYEE
WHERE EMP_ID IS NULL ; -- 사수의 번호가 실제로 존재하는 사람인지
...
-- 상관쿼리 ( 이태림이 D8부서에서 가장 빠른입사 / 퇴사자 걸러짐 - 08부서 아예 제외 )
SELECT EMP_ID, EMP_NAME , DEPT_CODE, NVL(DEPT_TITLE, '소속없음'), JOB_NAME, HIRE_DATE  
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE ENT_YN ='N' AND
HIRE_DATE = (SELECT MIN(HIRE_DATE)
									FROM EMPLOYEE SUB
									LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
...
NVL(
	(SELECT EMP_NAME FROM EMPLOYEE SUB WHERE SUB.EMP_ID = MAIN.MANAGER_ID ) , '없음') 
FROM EMPLOYEE MAIN ;
...
FROM EMPLOYEE 
ORDER BY SALARY DESC; -- 오름차순으로 급여조회

SELECT ROWNUM, EMP_NAME, SALARY 
FROM (SELECT EMP_NAME , SALARY 
		FROM EMPLOYEE 
		ORDER BY SALARY DESC) -- 해석순위 1위 FROM 절에서 전체 직원의 SALARY 내림차순 정렬
WHERE ROWNUM <= 5; -- 해석순위2순위 WHERE절에서 이제 가상컬럼의 1~5 행까지만 조회한다.
...
SELECT TO_CHAR(SALARY * (1+ NVL(BONUS, 0)) * 12, 'L999,999,999') "연봉"
FROM EMPLOYEE ;
