...
ORDER BY 사번 ASC ;
-- SQL Error [955] [42000]: ORA-00955: 기존의 객체가 이름을 사용하고 있습니다. ( OR REPLACE를 작성하지 않았으면)
...
-- VIEW를 생성할 때 사용된 컬럼에만 데이터가 삽입되었꼬
-- 반대로 사용되지 않는 컬럼(DEPT_TITLE)에는 NULL이 들어감 ! 
...
FROM DEPT_COPY2 WITH READ ONLY; +-- 읽기전용
...
-- NEXTVAL을 호출할 때 마다
-- INCREMENT BY에 작성된 수 만큼 증ㅈ가하는지 확인
SELECT  SEQ_TEST_NO.NEXTVAL FROM DUAL;
-- 처음 : 100 
-- 1회 : 105
-- 2회 : 110
...
INSERT INTO TB_TEST VALUES(SEQ_TEST_NO.NEXTVAL, '맹구');
