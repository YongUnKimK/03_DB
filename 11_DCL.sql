...
ALTER SESSION SET "_ORACLE_SCRIPT" = TRUE; ---11G 이전 문법 사용 허용 ( 계정명을 간단히 작성 가능)

-- [작성법]
-- CREATE USER 사용자명 IDENTIFIED BY 비밀번호;
CREATE USER kyu_sample IDENTIFIED BY 1234;
...
GRANT CREATE SESSION TO kyu_sample;
...
-- CREATE TABLE : 테이블 생성 권한 부족
-- + 데이터를 저장할 수 있는 공간 ( TABLESPACE ) 할당

-- 6. SYS 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO kyu_sample;

ALTER USER kyu_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- > 해당 계쩡은 지정된 권한을 이용해서 특정 역할을 갖게된다

-- (SYS) simple계정에 CONNECT , RESOURCE ROLEE 부여
GRANT CONNECT, RESOURCE TO kyu_sample;

-- CONNECT : OB 접속 관련 권한을 묶어둔 ROLE
-- RESOURCE : OB 사용을 위한 기본 객체 생성 권한을 묶어둔 ROLE

-- CONNECT : 롤이 포괄하고 잇는 권한
...
-- 1. (sample) 계정이 kh 계정의 EMPLOYEE 테이블 조회 

SELECT * FROM KH_kyu.EMPLOYEE;
...
GRANT SELECT ON EMPLOYEE TO kyu_sample;
...
REVOKE SELECT ON EMPLOYEE FROM kyu_sample;
