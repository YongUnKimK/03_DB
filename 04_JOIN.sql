...
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) --ON안에는 연결고리 삼는 컬럼명

-- 1. 내부 조인 ( == 등가조인 ( EQUAL JOIN )
 ...
 * -> 연결에 사용한 컬럼의 값이 일치하지 않으면 조회 결과에 포함X
 */
...
--> * 반드시 OUTER JOIN 명시해야 한다 ( 명시하지 않으면 INNER로 인식 )
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
...
-- 2) RIGHT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 오른편에
-- 기술된 테이블의 컬럼 수를 기준으로 JOIN 한다
...
FROM EMPLOYEE , DEPARTMENT 
WHERE DEPT_CODE(+) = DEPT_ID; 
-- SQL Error [1468] [72000]: ORA-01468: outer-join된 테이블은 1개만 지정할 수 있습니다 (+) FULL JOIN X 
...
-- SQL Error [918] [42000]: ORA-00918: 열의 정의가 애매합니다 (별칭 OR 테이블명.)
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
-- TIP) 같은 테이블 2개 있다고 생각하고 JOIN진행
-- 테이블마다 별칭 작성 ( 미작성시 열의정의가 애매하다는 오류 발생함)
...
SELECT EMP_ID,  JOB_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE e
JOIN JOB USING(JOB_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID  )
...
SELECT EMP_NAME , JOB_NAME, SALARY , (SALARY * NVL(BONUS, 1)) * 12
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL) 
...
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN LOCAL ON(LOCATION_ID = LOCAL_CODE )
JOIN 
