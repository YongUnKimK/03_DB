...
-- SQL Error [12899] [72000]: ORA-12899: "KH_KYU"."DEPT_COPY"."DEPT_ID" 
-- 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- DEPT_ID의 데이터 타입이 CHAR(2)이므로 영어+숫자 2글자까지저장가능
-- > VARCHAR2(3)으로 변경 ( 남는 바이트 메모리 반환 )
...
-- !!기존 데이터는 변경되지 않고, 앞으로 들어갈 기본값이 KOREA임 ( 여전히 한국 )
...
/* DDL 주의 사항
 * 
 * 1) DDL은 COMMIT / ROLLBACK이 되지 않는다.  ( 트랜잭션에 담기지않음 ) 
 * 
...
ROLLBACK; -- 롤백 X 위에서 DDL구문중 ALTER를 사용해서 그 시점에 COMMIT됨
