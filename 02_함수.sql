...
-- 함수는 SELECT 문의
-- SELECT 절, WHERE절, ORDER BY 절, GROUP BY 절, HAVING 절 사용 가능
...
-- 지정한 위치부터 지정한 순번대로 검색되는 문자의 위치를 반환
...
-- SYSDATE : 시스템에(년,월,일,시,분,초) 현재 시간을 반환
SELECT SYSDATE FROM DUAL;

-- SYSTIMESTAMP : SYSDATE + MS 단위 추가
SELECT SYSTIMESTAMP FROM DUAL;

-- MONTHS_BETWEEN(날짜, 날짜) : 전달받은 두 날짜의 개월 수 차이 반환
SELECT ABS(ROUND(MONTHS_BETWEEN( SYSDATE  , '2025-01-23'), 3)) "수강기간(개월)" FROM DUAL ; 
...
ABS(CEIL(MONTHS_BETWEEN(HIRE_DATE, SYSDATE)) ) "근무한 개월 수",
ABS(CEIL(MONTHS_BETWEEN(HIRE_DATE, SYSDATE) / 12)) || '년차' "근무년차"
FROM EMPLOYEE ;
...
SELECT LAST_DAY('476-02-01') FROM DUAL; 
...
-- 형변환 함수
-- 문자열(CHAR), 숫자(NUMBER), 날짜(DATE) 끼리 형변화 ㄴ가능

-- 문자열로 변환
-- TO_CHAR(날짜, [포맷]) : 날짜형 데이터를 문자형 데이터로 변경 
-- TO_CHAR(숫자, [포맷]) : 날짜형 데이터를 문자형 데이터로 변경 
...
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" ( DY )') 연습문제 FROM DUAL;
...
SELECT TO_DATE('241008 163219', 'YYMMDD HH24MISS') FROM DUAL;
...
--			절반(50) 미만인 경우 현재 세기(2000년대) 

SELECT TO_DATE('200712', 'RRMMDD') FROM DUAL; -- 2049년
SELECT TO_DATE('940712', 'RRMMDD') FROM DUAL; -- 1994년

-- EMPLOYEE 테이블에서 각 직원이 태어난 생년월일 조회
-- 사원이름, 생년월일(1965년 10월 08일)

SELECT SUBSTR(EMP_NO , 1, (INSTR(EMP_NO ,'-') -1) )FROM EMPLOYEE e ;
...
-- 숫자 형변환
-- TO_NUMBER(문자데이터, [포맷]) : 문자형 데이터를 숫자 데이터로 변환

SELECT TO_NUMBER('100,000,0') + 50000 FROM DUAL; --SQL Error [1722] [42000]: ORA-01722: 수치가 부적합합니다
...
-- NVM(컬럼명, 컬럼값이 NULL일 때 바꿀 값) : NULL인 컬링값을 다른값으로 변경
-- NULL과 산술 연산을 진행하면 NULL 발생
...
SELECT EMP_NAME, NVL2(BONUS,'0','X')
FROM EMPLOYEE e ;
...
-- SUM(숫자가 기록된 컬럼명) : 합계
-- 모든 직원의 급여 합 조회
SELECT SUM(SALARY) FROM EMPLOYEE e ; -- 70,096,240
...
-- 급여 최소값, 가장빠른 입사일, 알파벳순서가 가장빠른 이메이릉ㄹ 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE e ;

-- 급여 최대값, 가장늦은 인사말, 알파벳순서가 가장느린 이메일을 조회
