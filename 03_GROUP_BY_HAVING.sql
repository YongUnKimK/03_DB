...
-- 1) 부서코드만 조회
SELECT DEPT_CODE FROM EMPLOYEE e ; -- 23행

-- 2) 전체 급여 합 조회
SELECT  SUM(SALARY) FROM EMPLOYEE e ; -- 1행
...
SELECT JOB_CODE ,ROUND( AVG(SALARY) ) 평균, COUNT(*) 수
FROM EMPLOYEE 
GROUP BY JOB_CODE 
ORDER BY JOB_CODE  ;
...
SUM(SALARY) "급여 합"
FROM EMPLOYEE 
GROUP BY DECODE(SUBSTR(EMP_NO,8,1), '1', '남', '2', '여')  -- 별칭 사용 X
ORDER BY "인원 수";
...
-- 부서코드가 D5, D6인 부서의 ( WHERE 절 조건 -> GROUP BY절 )
-- 부서코드, 평균급여, 인원 수 조회 ( SELECT 절 )

SELECT DEPT_CODE , ROUND( AVG(SALARY) ) 평균급여, COUNT(*) 인원수
FROM EMPLOYEE 
...
-- EMPLOYEE 테이블에서
-- 부서별로 같은 직급인 사원의 수를 조회
-- 부서코드 오름차순, 직급코드 내림차순으로 정렬
-- 부서코드, 직급코드, 인원수

SELECT DEPT_CODE , JOB_CODE , COUNT(*)  인원 -- COUNT는 그룹함수 
FROM EMPLOYEE
GROUP BY DEPT_CODE, JOB_CODE -- DEPT_CODE로 그룹을 나누고,
							 -- 나눠진 그룹 내에서 JOB_CODE로 그룹을 분류
...
FROM EMPLOYEE 
-- WHERE AVG(SALARY) >= 3000000 --> 한 사람의 급여가 300만원 이상이라는 조건 ( 요구사항에 부합하지 않음 )
GROUP BY DEPT_CODE
...
-- 집계 함수( ROLLUP, CUBE)
-- 그룹 별 산출 결과 값의 집계를 계산하느 ㄴ함수
-- (그룹별로 중간 집계 결과를 추가)
-- GROUP BY 절에서만 사용할 수 있는 함수

-- ROLLUP : GROUP BY 절에서 가장먼저 작성된 칼럼의 중간 집계를 처리하는 함수
...
-- SQL Error [1789] [42000]: ORA-01789: 질의 블록은 부정확한 수의 결과 열을 가지고 있습니다. (컬럼의 개수를 뺄경우)
-- SQL Error [1790] [42000]: ORA-01790: 대응하는 식과 같은 데이터 유형이어야 합니다 ( 컬럼의 타입이 다를경우 )
